...
function StudentApp({ onLogout }: { onLogout: () => void }) {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case "courses": return <CourseCatalog onEnter={handleEnterCourse} />;
      case "course-player": return <CoursePlayer course={activeCourseData} onBack={() => setScreen("courses")} />;
      ...
      case "goal-roadmap": return <GoalRoadmap />;
      case "projects": return <Projects />;
      case "mentor": return <Mentor />;
      case "peers": return <PeerLearning />;
        ...
        <div className={`p-3 border-t border-slate-100 ${sidebarCollapsed ? "flex justify-center" : ""}`}>
          {!sidebarCollapsed ? (
            <div>
              ...
              <button onClick={onLogout} className="mt-2 w-full text-xs text-slate-400 hover:text-slate-700 text-left px-1">← Switch role</button>
            </div>
          ) : (
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center px-4 gap-3 flex-shrink-0">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button className="relative p-1.5 hover:bg-slate-100 rounded-[8px]">
              <Icon name="bell" stroke="#64748B" size={18} />
